...
public class graveBlock extends ContainerBlock {
  ...
  public graveBlock ( ) {
    super( AbstractBlock.Properties
        ...
        .harvestLevel( 1 )
        .setLightLevel( ( state ) -> state.get( WATERLOGGED ) ? 6 : 0 )
    );
  ...
  private static class graveBlockFactory {
    private NonNullList<ItemStack> inventory = NonNullList.withSize(41, ItemStack.EMPTY );
            ...
            if ( tileEntity instanceof graveEntity ) { // 是实体墓碑容器
              ...
              if ( named && this.playerName != null && this.playerUUID != null ) { // 有主
                ( (graveEntity) tileEntity )
                    ...
                    .displayMsg( this.msg )
                    .isBuild( false )
                    .setCustomName( new TranslationTextComponent( "txt.nanzhi_grave.g_o_name", playerName ) );
              } else { // 无主
                LocalDate date = LocalDate.now( ) ; // 获取系统时间
                ( (graveEntity) tileEntity )
                    .displayMsg( new TranslationTextComponent( "txt.nanzhi_grave.d_o_n",
                        date.getYear( ) ,
                        date.getDayOfYear( )
                    ) )
                    .isBuild( false ) ;
    ...
    private void __putItemStack ( ItemStack stack, int index, PlayerEntity player ) {
      ...
      if ( !config.free_nameless.get( ) && !skip && conIndex == -1 && ( item == Items.BARREL || item == Items.CHEST || item == Items.TRAPPED_CHEST ) ) {
        conIndex = index ;
        conStack = stack ;
      } ;
    } ;
  ...
  private static void notice ( PlayerEntity player, BlockPos pos, int msg ) {
    ...
    if ( ( msg & 1 ) == 1 ) { // 1: 成功消息
      player.sendStatusMessage( new TranslationTextComponent( "txt.nanzhi_grave.create_grave" ), false ) ;
    } ;
    if ( config.death_msg_pos.get( ) && ( ( msg >> 1 ) & 1 ) == 1 ) { // 2: 坐标点
      player.sendStatusMessage( new TranslationTextComponent( "txt.nanzhi_grave.create_grave_pos", pos.getX( ), pos.getY( ), pos.getZ( ) ), false ) ;
    } ;
    if ( ( ( msg >> 2 ) & 1 ) == 1 ) { // 4: 无法创建
      player.sendStatusMessage( new TranslationTextComponent( "txt.nanzhi_grave.c_c_g_u" ), false ) ;
    } ;
    if ( config.death_msg_not_found_container.get( ) && ( ( msg >> 3 ) & 1 ) == 1 ) { // 8: 找不到容器
      player.sendStatusMessage( new TranslationTextComponent( "txt.nanzhi_grave.c_c_g_c" ), false ) ;
    } ;
  ...
  public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
    if ( state.getBlock( ) == blockReg.graveBlock ) {
      ...
      if ( state != nState ) { // 更新
        ...
        // 墓主，或者人为建造的无主之墓可打开
        if ( ( entity.uuid( ) == null && entity.isBuild( ) ) || ( entity.uuid( ) != null && entity.uuid( ).equals( player.getUniqueID( ) ) ) ) {
          if ( config.break_useShove.get( ) && stack.getToolTypes( ).contains( ToolType.SHOVEL ) ) { // 试图摧毁
          ...
          player.sendStatusMessage( new TranslationTextComponent( "txt.nanzhi_grave.cannot_open" ), true );
  ...
  public void onBlockHarvested( World worldIn, BlockPos pos, BlockState state, PlayerEntity player ) {
    if ( !worldIn.isRemote( ) && config.spawn_zombie.get( ) && !player.isCreative( ) ) { // 读取配置且是服务器且玩家不是创造模式
      ...
      if ( tileEntity instanceof graveEntity ) {
        graveEntity entity = ( graveEntity ) tileEntity ;
        // 非建造坟墓且不是玩家的坟墓被破坏时
        if ( !entity.isBuild( ) && ( entity.uuid( ) == null || !entity.uuid( ).equals( player.getUniqueID( ) ) ) ) {
  ...
  public boolean isValidPosition( BlockState state, IWorldReader worldIn, BlockPos pos)  {
    ...
    return tags.stone.contains( ns ) || tags.dirt.contains( ns ) || tags.sand.contains( ns ) || ns.getBlock( ) instanceof graveBlock || ns.getBlock( ) instanceof OreBlock ;
  }
  @Override
  ...
  public TileEntity createNewTileEntity(IBlockReader worldIn) {
    return new graveEntity( new TranslationTextComponent( "block.nanzhi_grave.grave" ) );
  }
    ...
    if ( tileentity instanceof graveEntity ) {
      ...
      if ( placer instanceof PlayerEntity ) {
        ...
        grave_entity.setCustomName(
            new TranslationTextComponent( "txt.nanzhi_grave.grave", placer.getName( ).getString( ), sneaking ? new TranslationTextComponent( "block.nanzhi_grave.grave" ).getString( ) : new TranslationTextComponent( "txt.nanzhi_grave.g_o_n" ).getString( ) )
        ) ;
        // 自定义显示内容
  ...
  protected void fillStateContainer(StateContainer.Builder<Block, BlockState> builder) {
    builder.add( FACING ).add( WATERLOGGED, STONE ) ;
  }
